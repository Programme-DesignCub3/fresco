...
<template>
    <div v-if="popup" @click.self="closePopup"
        ...
        <div :class="themeStore.theme == 'black'
            ...
            <div class="row-span-3 flex items-center justify-center gap-3 lg:col-span-4 2xl:col-span-4">
                <img class="w-1/3 lg:w-5/6" :src="popup.image" :alt="popup.product" />
            </div>
            <div class="row-span-3 text-white lg:col-span-8 2xl:col-span-8">
                <div class="space-y-3">
                    <h3 class="text-[32px] font-semibold leading-none md:text-[40px] lg:text-[60px]">
                        Fresco {{ popup.product }}
                    </h3>
                    <div class="h-[170px] overflow-y-scroll leading-7 md:h-auto md:overflow-auto"
                        v-html="popup.content"></div>
                </div>
                <a :href="popup.links[0].link" target="_blank"
                    class="button yellow group-hover:yellow dark:group-hover:green dark:green mt-3 flex items-center">
                    <span class="flex items-center">
                        <Icon class="mr-2 h-4 w-4 stroke-2" icon="fa-solid:shopping-cart" />
                        BELI SEKARANG
                    </span>
                </a>
            </div>
