...
<template>
    <div v-show="show" @click.self="closePopup"
        ...
        <div class="relative flex w-full flex-col md:w-2/3 lg:w-[550px]">
            <img class="object-cover object-center" width="auto" height="auto" :src="data.image" :alt="data.title" />
            <div class="bg-linear-red space-y-1 rounded-xl px-6 py-4 md:px-8 md:py-6">
                <h2 class="home-popup-title">CONSUMER PROMO</h2>
                <h1 class="text-center text-3xl font-bold tracking-wider text-white md:text-5xl lg:text-7xl">
                    {{ data.title }}
                </h1>
                <div class="text-center text-xl font-semibold tracking-wider text-fr-yellow" v-html="data.description">
                </div>
            </div>
